...
\vspace{4pt}
\begin{enumerate}
    \item \textbf{Input:} Resume batch \(R\), job description \(J\), scoring weights \(W=(W_t,W_s,W_e,W_c)\), bonus cap \(B_{max}\), and context mode \(M \in \{\text{TF-IDF},\text{MiniLM}\}\).
    ...
    \begin{enumerate}
        ...
        \item Compute technical coverage, soft-skill coverage, capped experience match, and contextual similarity using the selected context mode \(M\).
...
\end{equation}
When a job description does not explicitly request soft skills, the soft component is assigned full base credit so that the candidate is not penalized by a zero denominator.
...
\end{equation}
The cap prevents a candidate with double the required experience from receiving a mathematically inflated 200\% experience score. Month-level evidence is preserved as a decimal year value, so a six-month profile can be displayed and scored as \(0.5/2\) when the job requires two years.

\subsection{Contextual Similarity}
The system supports two context-similarity modes. The default mode uses TF-IDF vectorization to represent the resume and job description as sparse vectors. The implementation uses scikit-learn's \texttt{TfidfVectorizer} with English stop-word removal, a maximum vocabulary size of 5000, and unigram-plus-bigram features \((1,2)\). The bi-gram setting captures short technical phrases such as ``machine learning'' without loading a neural embedding model. The advanced mode keeps the same rule-based skill extraction and scoring formula but replaces the context vector with an \texttt{all-MiniLM-L6-v2} sentence embedding. Let \(A\) be the resume context vector and \(B\) be the job-description context vector under the selected mode. Their contextual similarity is computed by cosine similarity:
\begin{equation}
S_{ctx}=\frac{A \cdot B}{\|A\|\|B\|}\times 100.
\end{equation}
TF-IDF rewards distinctive terms more strongly than generic words. In combination with cosine normalization, it reduces the benefit of simply making a resume longer or repeatedly inserting a subset of terms, which is useful for resisting simple keyword-stuffing attacks. MiniLM provides better handling of synonymous phrasing such as ``RESTful services'' and ``REST APIs'', but it requires additional model loading and inference time. The two modes therefore expose a practical trade-off rather than changing the bounded scoring model itself.
...
\end{split}
\end{equation}
The extra-skill bonus \(B\) is computed from technical or soft skills that appear in the resume but are not explicitly required by the job:
\begin{equation}
B=\min\left((|T_R-T_J|+|P_R-P_J|)\times \frac{B_{max}}{10},B_{max}\right).
\end{equation}
The cap prevents score inflation when candidates list many adjacent technologies or general soft-skill terms.

\subsection{Explanation and Complexity}
The explanation generator reports the match band, final score, matched experience ratio, matched JD skills, missing JD skills, bonus technical skills, bonus soft skills, and score breakdown. This deterministic template is intentionally direct: a recruiter can immediately see whether a candidate is missing PostgreSQL, REST, leadership, or another required capability. Let \(n\) be the number of resumes, \(d\) the TF-IDF vocabulary size, and \(k\) the skill ontology size. Digital parsing is approximately linear in document length, skill matching is proportional to ontology coverage, and final sorting is \(O(n\log n)\). OCR is the expensive branch, but it is invoked only for image files or failed digital extraction. This conditional design preserves speed while still reducing format bias.

\subsection{Feature Engineering Rationale}
The framework separates hard evidence from soft evidence. Explicit technical skills, required soft skills, and years of experience are treated as bounded requirements, while contextual similarity helps distinguish candidates with similar explicit-skill coverage. The extra-skill bonus is separated from the base score so that adjacent skills such as Docker, Redis, leadership, or public speaking add value without hiding missing required skills such as PostgreSQL, REST, or communication.
